...
async function data2buffer(data) {
    ...
    if (data.logs.length > 0) {
        ...
        const robotpos = (0, deepcopy_1.default)(data.robots);
        for (const v of (0, deepcopy_1.default)(data.logs).reverse()) {
...
async function uploadbuffer(image) {
    const result = await new Promise((resolve, reject) => {
        cloudinary_1.default.v2.uploader.upload_stream({ resource_type: 'image' }, (error, data) => {
            if (error) {
                reject(error);
            }
            else {
...
const upload = async (data) => {
    ...
    image = await (0, sharp_1.default)(image, data2rawsharp(data))
        .jpeg()
        .toBuffer();
