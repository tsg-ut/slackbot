...
exports.TwentyQuestions = void 0;
...
const gameStatusMessage_1 = __importDefault(require("./views/gameStatusMessage"));
const playerModal_1 = __importDefault(require("./views/playerModal"));
const gameLogModal_1 = __importDefault(require("./views/gameLogModal"));
const const_1 = require("./const");
const rankingUtils_1 = require("./rankingUtils");
const mutex = new async_mutex_1.Mutex();
const log = logger_1.default.child({ bot: 'twenty-questions' });
const TwentyQuestionsGames = firestore_1.default.collection('twenty_questions_games');
class TwentyQuestions {
    ...
    static async create(slack) {
        ...
        const state = await state_1.default.init('twenty-questions', {
            currentGame: null,
        });
    ...
    constructor(slack, state) {
        ...
        this.#interactions.viewSubmission('twenty_questions_player_modal', (payload) => {
    ...
    async startGame(userId) {
        ...
        const result = await this.#slack.chat.postMessage({
            channel: this.#SANDBOX_ID,
            text: '20の扉ゲーム開始！',
            blocks: (0, gameStatusMessage_1.default)(this.#state),
            username: '20の扉',
            icon_emoji: ':door:',
        });
    ...
    async generateTopicDescription(topic, ruby) {
        ...
        const completion = await openai_1.default.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: `あなたは「20の扉」の質問に答えるアシスタントです。正解となるお題は「${topic}」(読み: ${ruby})です。\n` +
                        `\n` +
    ...
    async selectTopic() {
        ...
        const candidateWords = await (0, candidateWords_1.getCandidateWords)({ min: 2, max: 10 });
        // Step 1: 200個の候補から選ぶ処理を10回繰り返す
        const selectedWords = [];
        for (let i = 0; i < 10; i++) {
            const sampledWords = (0, lodash_1.sampleSize)(candidateWords, 200);
            ...
            const completion = await openai_1.default.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'あなたは「20の扉」ゲームのお題を選ぶアシスタントです。' +
        ...
        const finalCompletion = await openai_1.default.chat.completions.create({
            model: 'gpt-4o-mini',
            messages: [
                {
                    role: 'system',
        ...
        const topic = (0, lodash_1.sample)(candidates);
    ...
    scheduleGameEnd(game) {
        const timeUntilEnd = game.startedAt + const_1.GAME_TIMEOUT - Date.now();
        if (timeUntilEnd <= 0) {
    ...
    async endGame() {
        ...
        if (correctPlayers.length > 0) {
            ...
            // 参加者数を計算
            const participantCount = Object.values(this.#state.currentGame.players).filter((p) => p.questionCount > 0).length;
            // 1位の全プレイヤーに実績を付与
            for (const player of firstPlacePlayers) {
                await (0, achievements_1.increment)(player.userId, 'twenty-questions-first-place');
                // 5人以上参加している場合
                if (participantCount >= 5) {
                    await (0, achievements_1.increment)(player.userId, 'twenty-questions-first-place-5plus-players');
                    // 唯一の正解者である場合
                    if (correctPlayers.length === 1) {
                        await (0, achievements_1.increment)(player.userId, 'twenty-questions-only-correct-player-5plus-players');
                    }
                }
            }
        }
    ...
    async saveGameToFirestore(game) {
        ...
        await TwentyQuestionsGames.add({
            ...
            startedAt: firebase_admin_1.firestore.Timestamp.fromMillis(game.startedAt),
            finishedAt: firebase_admin_1.firestore.Timestamp.fromMillis(game.finishedAt),
            statusMessageTs: game.statusMessageTs,
            players,
        });
        log.info(`Game ${game.id} saved to Firestore`);
    }
    async handleJoinButton(payload) {
        if (!this.#state.currentGame || this.#state.currentGame.status === 'finished') {
            await this.#slack.chat.postEphemeral({
        ...
        if (!this.#state.currentGame.players[userId]) {
            ...
            await (0, achievements_1.increment)(userId, 'twenty-questions-participate');
        }
        const player = this.#state.currentGame.players[userId];
        await this.#slack.views.open({
            trigger_id: payload.trigger_id,
            view: (0, playerModal_1.default)(this.#state, player),
        });
    }
    async handleViewLogButton(payload) {
        ...
        await this.#slack.views.open({
            trigger_id: payload.trigger_id,
            view: (0, gameLogModal_1.default)(gameData),
        });
    }
    ...
    async handleQuestionSubmit(payload) {
        if (!this.#state.currentGame || this.#state.currentGame.status === 'finished') {
            return;
        }
        const userId = payload.user.id;
    ...
    async handleAnswerSubmit(payload) {
        if (!this.#state.currentGame || this.#state.currentGame.status === 'finished') {
            return;
        }
        const userId = payload.user.id;
    ...
    async handleQuestion(userId, player, question, viewId) {
        if (!this.#state.currentGame) {
            return;
        }
        // 長さ制限のチェック
        if (question.length > const_1.MAX_QUESTION_LENGTH) {
        ...
        const completion = await openai_1.default.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: `あなたは「20の扉」ゲームのアシスタントです。正解となるお題は「${topic}」です。\n` +
                        `\n` +
        ...
        if (duplicateIndices.length > 0) {
            await (0, achievements_1.increment)(userId, 'twenty-questions-duplicate-question');
            // 異なる回答を得た場合
            if (duplicateIndices.some((duplicateIndex) => actualQuestions[duplicateIndex].answer !== aiAnswer)) {
                await (0, achievements_1.increment)(userId, 'twenty-questions-duplicate-question-different-answer');
            }
            // 10問以上前の質問と重複
            if (duplicateIndices.some((duplicateIndex) => duplicateIndex < actualQuestions.length - 10)) {
                await (0, achievements_1.increment)(userId, 'twenty-questions-duplicate-question-10plus-ago');
            }
            // 直前の質問と重複
            if (duplicateIndices.some((duplicateIndex) => duplicateIndex === actualQuestions.length - 1)) {
                await (0, achievements_1.increment)(userId, 'twenty-questions-duplicate-question-consecutive');
            }
        }
        ...
        if (player.questionCount >= const_1.MAX_QUESTIONS) {
    ...
    async handleAnswer(userId, player, answer, viewId) {
        if (!this.#state.currentGame) {
            return;
        }
        // 長さ制限のチェック
        if (answer.length > const_1.MAX_ANSWER_LENGTH) {
        ...
        const completion = await openai_1.default.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: `あなたは「20の扉」ゲームの回答を判定するアシスタントです。` +
        ...
        const isCorrect = answer.trim().toUpperCase() !== 'YES' &&
        ...
        else if (player.questionCount >= const_1.MAX_QUESTIONS) {
            await this.finishPlayer(userId, player, false, viewId);
        }
        else if (viewId) {
            await this.updatePlayerModal(viewId, player);
    ...
    async finishPlayer(userId, player, isCorrect, viewId) {
        if (!this.#state.currentGame) {
            return;
        }
        ...
        if (isCorrect) {
            ...
            await (0, achievements_1.increment)(userId, 'twenty-questions-correct');
            await (0, achievements_1.increment)(userId, `twenty-questions-correct-${player.questionCount}`);
            if (player.questionCount <= 10) {
                await (0, achievements_1.increment)(userId, 'twenty-questions-correct-within-10');
            }
            if (player.questionCount <= 15) {
                await (0, achievements_1.increment)(userId, 'twenty-questions-correct-within-15');
            }
            const actualQuestions = player.questions.filter((q) => !q.isAnswerAttempt);
            if (actualQuestions.length > 0) {
                ...
                if (allYes) {
                    await (0, achievements_1.increment)(userId, 'twenty-questions-correct-all-yes');
                }
                // すべて「いいえ」で正解
                const allNo = actualQuestions.every((q) => q.answer === 'いいえ');
                if (allNo) {
                    await (0, achievements_1.increment)(userId, 'twenty-questions-correct-all-no');
                }
                // 「わかりません」が5回以上
                const wakaranaiCount = new Set(actualQuestions
                    .filter((q) => q.answer === 'わかりません')
                    .map((q) => this.normalizeAnswer(q.question))).size;
                if (wakaranaiCount >= 5) {
                    await (0, achievements_1.increment)(userId, 'twenty-questions-correct-5plus-wakaranai');
                }
                // 「答えられません」が5回以上
                const kotaerarenaiCount = new Set(actualQuestions
                    .filter((q) => q.answer === '答えられません')
                    .map((q) => this.normalizeAnswer(q.question))).size;
                if (kotaerarenaiCount >= 5) {
                    await (0, achievements_1.increment)(userId, 'twenty-questions-correct-5plus-kotaerarenai');
                }
                // 「どちらともいえない」が5回以上
                const dochiraCount = new Set(actualQuestions
                    .filter((q) => q.answer === 'どちらともいえない')
                    .map((q) => this.normalizeAnswer(q.question))).size;
                if (dochiraCount >= 5) {
                    await (0, achievements_1.increment)(userId, 'twenty-questions-correct-5plus-dochira');
                }
            }
        }
        else {
            ...
            await (0, achievements_1.increment)(userId, 'twenty-questions-fail');
        }
        await this.updateStatusMessage();
    }
    ...
    async updatePlayerModal(viewId, player) {
        try {
    ...
    generateGameEndBlocks() {
        ...
        const allPlayers = Object.values(this.#state.currentGame.players).filter((p) => p.questionCount > 0);
        ...
        if (correctPlayers.length > 0) {
            const rankedPlayers = (0, rankingUtils_1.getRankedPlayers)(correctPlayers);
            for (const { player, rank } of rankedPlayers) {
                const rankEmoji = (0, rankingUtils_1.getRankEmoji)(rank);
                blocks.push({
                    type: 'section',
                    text: {
                        type: 'mrkdwn',
        ...
        if (incorrectPlayers.length > 0) {
            ...
            for (const player of incorrectPlayers) {
                const lastAnswerText = (0, lodash_1.last)(player.questions.filter((q) => q.isAnswerAttempt));
...
exports.TwentyQuestions = TwentyQuestions;
