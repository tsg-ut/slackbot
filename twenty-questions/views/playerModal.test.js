...
describe('playerModal', () => {
    ...
    it('shows question input when player has not finished', () => {
        const view = (0, playerModal_1.default)(baseState, basePlayer);
        expectEquals(view.type, 'modal');
        expect(view.title?.text).toBe('20の扉');
        expect(view.submit?.text).toBe('質問を送信');
        // 質問入力ブロックが存在することを確認
        const questionBlock = view.blocks?.find((block) => block.type === 'input' && 'block_id' in block && block.block_id === 'question_input');
    ...
    it('hides question input when player has 19 questions', () => {
        ...
        const view = (0, playerModal_1.default)(baseState, player);
        expectEquals(view.type, 'modal');
    ...
    it('shows answer input when player has questions', () => {
        ...
        const view = (0, playerModal_1.default)(baseState, player);
        // 回答入力ブロックが存在することを確認
        const answerBlock = view.blocks?.find((block) => block.type === 'input' && 'block_id' in block && block.block_id === 'answer_input');
    ...
    it('displays question history', () => {
        ...
        const view = (0, playerModal_1.default)(baseState, player);
        // 質問履歴が表示されていることを確認
        const historyBlock = view.blocks?.find((block) => block.type === 'section' && 'text' in block && block.text?.text?.includes('Q1:'));
