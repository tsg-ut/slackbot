...
exports.default = (game) => {
    ...
    // 正解者を質問回数順にソート
    const correctPlayers = (0, lodash_1.sortBy)(game.players.filter((p) => p.score !== null), (p) => p.score);
    // 未正解者（質問したが正解できなかった）
    const failedPlayers = game.players.filter((p) => p.questionCount > 0 && p.score === null);
    // 全てのプレイヤーを表示（正解者が先）
    ...
    if (allPlayers.length === 0) {
        ...
        for (const player of allPlayers) {
            ...
            const statusText = player.score !== null
                ? `${player.score}問で正解`
                : '不正解';
            blocks.push({
                type: 'section',
