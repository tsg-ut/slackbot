...
const loaderNamori = new Loader<CharacterData[]>(async () => [
  ...(await loadCharacters('namori')),
]);

const loaderIxy = new Loader<CharacterData[]>(async () => [
    ...
    mutex.runExclusive(async () => {
      if (
        ...
        const candidateCharacterIds = characters
          .filter(
            (character) =>
              !persistentState.recentCharacterIds.includes(
