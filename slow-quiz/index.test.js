...
describe('slow-quiz', () => {
    ...
    beforeEach(async () => {
        ...
        jest.mocked(openai_1.default.chat.completions.create).mockResolvedValue({
            choices: [{
        ...
        // @ts-expect-error: Mocking only the necessary properties
        jest.mocked(openai_1.default.batches.create).mockResolvedValue({
            id: 'batch-123',
        });
        // @ts-expect-error: Mocking only the necessary properties
        jest.mocked(openai_1.default.batches.retrieve).mockResolvedValue({
            status: 'completed',
        });
        // @ts-expect-error: Mocking only the necessary properties
        jest.mocked(openai_1.default.files.create).mockResolvedValue({
            id: 'file-123',
        });
        slowQuiz = new index_1.SlowQuiz({
    ...
    describe('validateQuestion', () => {
        it('should validate short questions correctly', () => {
            expect((0, index_1.validateQuestion)('日本一高い山は何？')).toBe(true);
            expect((0, index_1.validateQuestion)('a'.repeat(90))).toBe(true);
            expect((0, index_1.validateQuestion)('a'.repeat(91))).toBe(false);
        });
        it('should handle questions with special brackets', () => {
            expect((0, index_1.validateQuestion)('日本一高い山【山梨県と静岡県の境にある】は何？')).toBe(true);
            expect((0, index_1.validateQuestion)(`【説明文】${'a'.repeat(90)}`)).toBe(true);
        });
        it('should validate multi-part questions with slashes', () => {
            expect((0, index_1.validateQuestion)('日本で/一番/高い/山は/何？')).toBe(true);
            expect((0, index_1.validateQuestion)(`${'a'.repeat(20)}/${'b'.repeat(20)}/${'c'.repeat(20)}/${'d'.repeat(20)}/${'e'.repeat(20)}`)).toBe(true);
            const manyParts = Array(91).fill('a').join('/');
            expect((0, index_1.validateQuestion)(manyParts)).toBe(false);
        });
        it('should handle edge cases', () => {
            expect((0, index_1.validateQuestion)('')).toBe(true);
            expect((0, index_1.validateQuestion)('/')).toBe(true);
            expect((0, index_1.validateQuestion)(`a/b/c/${'d'.repeat(85)}`)).toBe(false);
        });
    });
    describe('SlowQuiz class', () => {
        ...
        describe('game selection logic', () => {
            it('should prioritize users who have never been selected', async () => {
                ...
                const waitlistedGameFromOldUser = {
                    ...getBaseGame(),
                ...
                const waitlistedGameFromNewUser = {
                    ...getBaseGame(),
