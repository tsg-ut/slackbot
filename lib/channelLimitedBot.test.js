...
const channelLimitedBot_1 = require("./channelLimitedBot");
...
describe('ChannelLimitedBot', () => {
    ...
    beforeEach(() => {
        ...
        slack.webClient.chat.getPermalink.mockResolvedValue({
            ok: true,
            permalink: 'https://slack.com/archives/CHANNEL_ID/p1234567890123456',
        });
        slack.webClient.chat.postEphemeral.mockResolvedValue({
            ok: true,
        ...
        slack.webClient.chat.delete.mockResolvedValue({
            ok: true,
        });
    });
    it('responds to messages containing the wake word in the allowed channel', async () => {
        const onWakeWord = jest.fn();
        class TestBot extends channelLimitedBot_1.ChannelLimitedBot {
            wakeWordRegex = /wakeword/;
            allowedChannels = [slack.fakeChannel];
        ...
        expect(onWakeWord).toHaveBeenCalledWith(expect.objectContaining({ text: 'wakeword in allowed channel' }), slack.fakeChannel);
    });
    it('posts ephemeral message and deletes original when wake word is used in disallowed channel (null response)', async () => {
        ...
        expect(onWakeWord).toHaveBeenCalledTimes(1);
        expect(onWakeWord).toHaveBeenCalledWith(expect.objectContaining({ text: 'wakeword in disallowed channel' }), slack.fakeChannel);
    ...
    it('posts ephemeral message with permalink and deletes original when wake word is used in disallowed channel (with response)', async () => {
        ...
        class TestBot extends channelLimitedBot_1.ChannelLimitedBot {
            ...
            allowedChannels = [slack.fakeChannel];
            onWakeWord = onWakeWord;
            progressMessageChannel = progressChannel;
        }
        new TestBot(slack);
