...
describe('State', () => {
    describe('StateDevelopment', () => {
        it('writes default values as JSON when initialization', async () => {
            await state_1.StateDevelopment.init('test', { number: 334 });
            expect(fs_extra_1.default.mkdirp.mock.calls).toHaveLength(1);
            expect(fs_extra_1.default.writeFile.mock.calls).toHaveLength(1);
            expect(fs_extra_1.default.writeFile.mock.calls[0]).toEqual([
                path_1.default.join(__dirname, '__state__', 'test.json'),
                '{\n  "number": 334\n}',
            ]);
        });
        it('makes state data saved to local state', async () => {
            const state = await state_1.StateDevelopment.init('test', {
                number: 334,
                list: [{ a: 0, b: '0' }],
            });
        ...
        it('saves JSON with updated data', async () => {
            ...
            const call = await new Promise((resolve) => {
                fs_extra_1.default.writeFile.mockImplementationOnce((...args) => resolve(args));
                state.number = 100;
                expect(state.number).toBe(100);
            });
            expect(call).toEqual([
        ...
        it('merges the saved state with the default value', async () => {
            fs_extra_1.default.pathExists.mockImplementation(async () => true);
            fs_extra_1.default.readFile.mockImplementation(async () => (JSON.stringify({ number: 100, list: [{ a: 1, b: '1' }] })));
            const state = await state_1.StateDevelopment.init('test', {
                number: 200,
                number2: 300,
                list: [{ a: 0, b: '0' }],
