...
const ARCHIVE_PROPOSAL_MESSAGE = (0, common_tags_1.stripIndent) `
	<!channel> このチャンネルには${ARCHIVE_LIMIT_DAYS}日以上BOT以外のメッセージが投稿されていません。
	引き続きこのチャンネルを使用しますか?
`;
const ARCHIVE_NOTE_MESSAGE = (0, common_tags_1.stripIndent) `
...
`;
const log = logger_1.default.child({ bot: 'auto-archiver' });
exports.default = async ({ eventClient, webClient: slack, messageClient: slackInteractions }) => {
    const channels = await state_1.default.init('auto-archiver_channels', {});
    const state = await state_1.default.init('auto-archiver_state', {
        snoozes: [],
        notices: [],
    });
    eventClient.on('message', (messageEvent) => {
    ...
    const postArchiveProposal = async (now) => {
        ...
        for (const channel of allChannels) {
            ...
            if (state.snoozes.some((snooze) => (snooze.channelId === channel.id &&
    ...
    node_schedule_1.default.scheduleJob('0 9 * * *', (now) => (mutex.runExclusive(() => (postArchiveProposal(now)))));
};
