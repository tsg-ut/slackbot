...
const countHit = (call, answer) => {
    (0, assert_1.default)(call.length === answer.length);
    const hits = new Set();
    for (let i = 0; i < call.length; i++) {
        if (call[i] === answer[i]) {
            hits.add(call[i]);
...
// Hitも合わせて数える
const countBlow = (call, answer) => {
    (0, assert_1.default)(call.length === answer.length);
    const blows = new Set();
    const callArray = Array(10).fill(0);
    const ansArray = Array(10).fill(0);
    for (let i = 0; i < call.length; i++) {
        callArray[call[i]]++;
        ansArray[answer[i]]++;
    }
...
const generateHistoryString = ({ call, hitsCount, blowsCount, }) => {
    return `${call
        .map((dig) => String(dig))
        .join('')}: ${hitsCount} Hit ${blowsCount} Blow`;
};
const answerLength2TimeLimit = (answerLength) => {
...
exports.default = ({ eventClient, webClient: slack, }) => {
    ...
    const postHistory = async (history) => {
      ...
      コール履歴: \`\`\`${history
                    .map((hist) => generateHistoryString(hist))
                    .join('\n')}\`\`\`
      `,
                channel: state.channel, // これが呼び出される時点ではchannelはnullにならないはず
                username: 'Hit & Blow',
    ...
    eventClient.on('message', async (message) => {
        if (!(0, slackUtils_1.isPlayground)(message.channel)) {
            return;
        }
        ...
        if (message.text.match(/^hitandblow( \d+)?$/)) {
            if (state.inGame) {
                const ongoingUrl = `https://tsg.slack.com/archives/${state.channel}/p${state.thread.replace('.', '')}`;
                await slack.chat.postMessage({
                ...
                const answerLength = rawAnswerLength !== undefined ? parseInt(rawAnswerLength) : 4;
                if (answerLength <= 0 || 10 < answerLength) {
                    ...
                    // state を更新してゲームを開始
                    state.inGame = true;
                    state.answer = (0, lodash_1.shuffle)((0, lodash_1.range)(10)).slice(0, answerLength);
            ...
            スレッドに数字でコールしてね`,
                    ...
                    // 実績解除
                    (0, achievements_1.unlock)(message.user, 'hitandblow-play');
                }
            }
        }
        // ゲーム中のスレッドでのみ反応
        if (message.thread_ts === state.thread) {
            ...
            if (message.text.match(/^\d+$/)) {
                ...
                const call = [...message.text].map((dig) => parseInt(dig));
                if (call.length !== state.answer.length) {
                    ...
                    if (!isValidCall(call)) {
                        ...
                        await slack.chat.postMessage({
                            text: `\`${call.map((dig) => String(dig)).join('')}\`: ${hits.size} Hit ${blows.size - hits.size} Blow`, // ここもgenerateHistoryStringとまとめようと思ったけど、ここ一箇所のために``用の分岐を入れるのもなんか違う気がしてる
                            channel: state.channel,
                            username: 'Hit & Blow',
                            icon_emoji: '1234',
                            thread_ts: state.thread,
                ...
                経過時間: ${(0, lodash_1.round)(passedTime / 1000, 3).toFixed(3)}秒`,
                                channel: state.channel,
                                username: 'Hit & Blow',
                                icon_emoji: '1234',
                                thread_ts: state.thread,
                            ...
                            postHistory(state.history);
                            // 実績解除
                            await (0, achievements_1.unlock)(message.user, 'hitandblow-clear');
                            if (state.answer.length >= 6) {
                                await (0, achievements_1.unlock)(message.user, 'hitandblow-clear-6digits-or-more');
                            }
                            if (state.answer.length === 10) {
                                await (0, achievements_1.unlock)(message.user, 'hitandblow-clear-10digits');
                            }
                            if (state.answer.length >= 3 && state.history.length === 1) {
                                await (0, achievements_1.unlock)(message.user, 'hitandblow-clear-once-3digits-or-more');
                            }
                            if (state.answer.length === 10 && passedTime <= 5 * 60 * 1000) {
                                await (0, achievements_1.unlock)(message.user, 'hitandblow-clear-10digits-within-5min');
                            }
                            // 終了処理
                            state.clear();
                        }
        ...
        if (message.text.match(/^hbdiff \d+ \d+$/)) {
            ...
            const call1 = [...rawCall1].map((dig) => parseInt(dig));
            const call2 = [...rawCall2].map((dig) => parseInt(dig));
