...
class CoordAteQuiz extends AteQuiz {
    ...
    judge(answer, _user) {
        ...
        if (Math.cos(xm - xmans) >= Math.cos(dist) &&
    ...
    solvedMessageGen(post) {
        ...
        message.text = `<@[[!user]]> 正解:tada:\n中心点の座標は <https://maps.google.co.jp/maps?ll=${latans},${lngans}&q=${latans},${lngans}&t=k|${latLngFormat(this.problem.answer[0], this.problem.answer[1])}> 、中心点までの距離は${distFormat(distance)}だよ:muscle:
			`;
        message.text = message.text.replaceAll(this.replaceKeys.correctAnswerer, post.user);
        return message;
    }
...
function countriesListMessageGen(aliases) {
    ...
    const mesCountries = {
        ...
        blocks: [
            ...
            ...Array.from(aliasesStringArray, (text) => {
...
function countriesExpand(countriesOriginal, aliases) {
    ...
    while (countries.length !== countriesLength) {
        ...
        for (let i = 0; i < countriesLength; i++) {
            ...
            if (countries[i].startsWith("-") &&
                aliases.hasOwnProperty(countries[i].slice(1))) {
                countries.push(...Array.from(aliases[countries[i].slice(1)], (x) => "-" + x));
                countries[i] = "-";
            }
        }
    }
...
function countriesListGen(aliases) {
    const arr = [];
    const aliasesString = [];
...
async function puppeteerWindow(latitude, longitude, zoom) {
    ...
    const maxZoom = (await page.evaluate("result"));
...
function latLngDeformat(str) {
    ...
    if (!lat_result)
        return null;
    str = str.replace(new RegExp(lat_result.sign + "\\s*" + lat_result.number, "i"), "");
    ...
    if (lat_sign === "s" || lat_sign === "-")
        lat = -lat;
    if (lng_sign === "w" || lng_sign === "-")
        lng = -lng;
    return [lat, lng];
}
function latLngToMercator(lat, lng) {
    const x = (lng / 180) * Math.PI;
    const y = Math.log((1 + Math.sin((lat / 180) * Math.PI)) /
        (1 - Math.sin((lat / 180) * Math.PI))) / 2;
    return [x, y];
}
...
function measureDistance(lat, lng, latans, lngans) {
...
function randomPoint(extent) {
    ...
    const lat = rad2Deg(Math.asin(Math.random() * (Math.sin(deg2Rad(maxy)) - Math.sin(deg2Rad(miny))) +
        Math.sin(deg2Rad(miny))));
    return [lat, lng];
}
function distFormat(num) {
    if (num >= 10) {
...
function sizeExtract(text) {
    const size = text.match(reNum) === null
        ? 1000
...
function countriesExtract(text, aliases) {
    const countriesList = countriesExpand(["世界"], aliases)
        .filter((country) => country !== "-")
        .sort();
    let countriesOriginal = text
        .slice(4)
    ...
    for (let i = 0; i < countries.length; i++) {
        if (countries[i] !== "-" &&
            ...
            errorTextArray.push(`「${countriesOriginal[i] === undefined
                ? countries[i]
                : countriesOriginal[i]}」という国・地域はないよ:anger:`);
        }
        else if (countries[i] !== "-") {
            validCounter += 1;
        }
...
async function problemGen(size, worldFilter) {
    let country, zoom, img_url, latitude, longitude;
    const extent = Turf.bbox(worldFilter);
    while (true) {
        [latitude, longitude] = randomPoint(extent);
        const points = Turf.points([[longitude, latitude]]);
        const resArr = worldFilter.features.filter((country) => Turf.pointsWithinPolygon(points, country).features.length > 0);
        if (resArr.length > 0) {
            country = resArr[0];
            zoom = Math.log2((((radius_of_earth * 1000 * 2 * Math.PI) / 256) *
                img_size *
...
function problemFormat(country, zoom, size, img_url, latitude, longitude, thread_ts) {
...
async function prepareProblem(slack, message, aliases, world, thread_ts) {
    ...
    worldFilter.features = worldFilter.features.filter((feature) => countries.includes(feature.properties.NAME_JA));
    const [country, zoom, sizeActual, img_url, latitude, longitude] = await problemGen(sizeOrig, worldFilter);
    const problem = problemFormat(country, zoom, sizeActual, img_url, latitude, longitude, thread_ts);
    return problem;
}
exports.default = async ({ eventClient, webClient: slack }) => {
    const aliases = (await fs_extra_1.default.readJson(__dirname + "/country_names.json"));
    const world = await fs_extra_1.default.readJson(__dirname + "/countries.geojson");
    eventClient.on("message", async (message) => {
        ...
        const [result, startTime, size] = await mutex.runExclusive(async () => {
            const arr = await Promise.race([
                (async () => {
                    const problem = await prepareProblem(slack, message, aliases, world, message.ts);
        ...
        if (result.state === "solved") {
            await (0, achievements_1.increment)(result.correctAnswerer, "coord-quiz-easy-answer");
            if (size < 20.00001) {
                await (0, achievements_1.increment)(result.correctAnswerer, "coord-quiz-professional-answer");
            }
            if (size <= 100.00001) {
                await (0, achievements_1.increment)(result.correctAnswerer, "coord-quiz-hard-answer");
            }
            if (size <= 500.00001) {
                await (0, achievements_1.increment)(result.correctAnswerer, "coord-quiz-medium-answer");
            }
            if (endTime - startTime <= 30000) {
                await (0, achievements_1.increment)(result.correctAnswerer, "coord-quiz-30sec-answer");
            }
        }
    });
};
