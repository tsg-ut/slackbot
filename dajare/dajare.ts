...
const isGoodWord = (text: string) => !(/^[ァィゥェォッャュョヮンー]/).test(text);
 ...
 */
const product = function* product(...arrays: any[][]): Generator<any[], void, unknown> {
	if (arrays.length === 0) {
		yield [];
		return;
	}
...
const substRulesList = Array.from(product(
	...
	[
		[],
		[
			[/[ァィゥェォッャュョヮ]/g, (char: string) => String.fromCodePoint((char.codePointAt(0) || 0) + 1)],
		],
	],
	[
		[],
...
export function findDajare(text: string, lim = 2) {
	...
	const isGoodIndices = (indices: number[], length: number) => (
		indices.length >= 2 &&
		(indices.slice(-1)[0] - indices[0]) >= length
	);
	const getNonOverlappingIndices = (indices: number[], length: number) => {
		if (indices.length === 0) {
			return [];
		}
...
export function listAlternativeReadings(readings: string[]) {
	const altReadings = substRulesList.map((rules) => readings.map((reading) => {
		let result = reading;
		for (const [regexp, repl] of rules as any) {
