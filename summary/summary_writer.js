...
module.exports.makeSummary = async (messages, slack) => {
    if (!fontloaded) {
        const fontPath = path.resolve(__dirname, '../lib/NotoSerifCJKjp-Bold.otf');
    ...
    // 瞬間最高コメント速度 (分刻み)
    {
        ...
        const [strongestTime, strongestRecord] = maxBy(
            Object.entries(minMessages), ([, records]) => records.filter(rec => rec.user).length
        );
        summary.push({
            ...
            text: strongestRecord.slice(0,5).map(rec => `${getMemberName(rec.user)}: ${rec.text}`).join('\n'),
        });
    }
    ...
    // コメント速度推移
    {
        ...
        const xaxis = "0:|" + halfHourMessages.map(([time,]) => {
            ...
            if (mom.minutes() == 0 && mom.hour() % 2 == 0) return mom.format('HH:mm');
            else return "";
        }).join("|");
        const maxlen = halfHourMessages.reduce((acc,x)=>Math.max(acc, x[1]), 0);
        const data = "t:" + halfHourMessages.map(([, len]) => (100*len/maxlen).toFixed(2)).join(",");
    ...
    // wordcloud
    {
        ...
        const stop_words = [ 'てる', 'いる', 'なる', 'れる', 'する', 'ある', 'こと', 'これ', 'さん', 'して',
                 'くれる', 'やる', 'くださる', 'そう', 'せる', 'した',  '思う',
                 'それ', 'ここ', 'ちゃん', 'くん', 'ない', 'ので', 'よう', ',', '.', '、', '。', '(', ')', '（', '）'].concat(hiraganaLetters)
        const nestedWordsPromise = filtering(messages).map(async (mes) => await (
            tokenize(mes.text)
                .then(res => res.reduce((acc, e) => {
                    if (e.pos === '名詞' &&
                            acc.length > 0 &&
                            acc[acc.length-1].pos === '名詞' &&
                            !stop_words.includes(e.surface_form) &&
                            !stop_words.includes(acc[acc.length-1].surface_form)) {
        ...
        const wordsCount = await Promise.all(nestedWordsPromise)
            ...
            .then(xs => {xs.sort((a, b) => (b[1] - a[1])); return xs.slice(0, WORDS_LIMIT);})
            //.then(xs => {logger.info(xs.toString()); return xs;})
            .then(xs => xs.map(([text, size]) => {return {text, size};}));
        ...
        const canPromise = new Promise((resolve, reject) => {
            ...
            image.onload = () => {
                ...
                canvas.toBuffer((err, buf) => {
                    if (err) {logger.error("error toBuffer"); reject(err);}
                    else resolve(buf);
                });
            };
