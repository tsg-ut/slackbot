...
exports.scrapbox2slack = void 0;
const axios_1 = __importDefault(require("axios"));
const logger_1 = __importDefault(require("../lib/logger"));
const log = logger_1.default.child({ bot: 'scrapbox' });
const getScrapboxUrl = (pageName) => `https://scrapbox.io/api/pages/tsg/${pageName}`;
const scrapbox2slack = (s) => (s.replace(/\[(?<url>https?:\/\/.+?)\]/g, '$<url>') // 外部リンク
...
exports.scrapbox2slack = scrapbox2slack;
exports.default = ({ webClient: slack, eventClient: event }) => {
    event.on('link_shared', async ({ links, message_ts, channel }) => {
        ...
        for (const link of scrapboxLinks) {
            ...
            const response = await axios_1.default.get(scrapboxUrl, { headers: { Cookie: `connect.sid=${process.env.SCRAPBOX_SID}` } });
            const { data } = response;
            ...
            const descriptions = rawDescriptions.map((d) => (0, exports.scrapbox2slack)(d));
            unfurls[url] = {
                title: data.title,
                title_link: url,
