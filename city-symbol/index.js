...
const prefectures_1 = require("../room-gacha/prefectures");
const chakuwiki_title_map_json_1 = __importDefault(require("./chakuwiki-title-map.json"));
const chakuwikiTitleMap = new Map(Object.entries(chakuwiki_title_map_json_1.default));
const mutex = new async_mutex_1.Mutex();
const log = logger_1.default.child({ bot: 'city-symbol' });
const promptTemplate = (0, common_tags_1.stripIndent) `
	# 指示

	{{cityname}}が答えになるクイズを作るとして、答えのヒントになるような短い文章を3つ作成してください。まず、{{cityname}}に関してあなたが知っている情報と、以下に示す{{cityname}}のブリタニカ百科事典・Chakuwiki・Wikipedia記事の内容をもとに、{{cityname}}に関する基本的な情報に関する辞書的な説明文を作成してください。続いて、{{cityname}}に関するニュースなどをもとに、有名な事実や面白いトリビアなどの情報をまとめてください。特に、以下の「ヒントとして積極的に含めるべき情報」に挙げた情報について優先的に列挙してください。次に、これらの情報から適切に取捨選択し、ヒントとして適切になるように組み合わせ、答えに導くような短いヒントを作成してください。ヒントには{{cityname}}に関連する固有名詞をなるべく多く含めてください。最後の行に、作成した3つのヒントを、string[]型を持つJSONの文字列の配列として出力してください。

	## ヒントとして積極的に含めるべき情報

	* この市町村が日本一・世界一であることがら
	* この市町村の観光地や名所
	* この市町村の二つ名・別名・愛称・キャッチフレーズ
	* この市町村の特産品や名物料理・B級グルメ・お土産
	* この市町村出身の有名人（誰でも知っている著名な人物に限る）
	...
	* ヒント1がもっとも平易で直接的なヒントであり、ヒント3に進むにつれて、より市町村を連想しにくい難しい情報が含まれている。ヒント3が最も難しい情報を含んでいる。
	...
	{{wikipedia_content}}
`;
const extractWikipediaPlaintext = (content) => {
...
const getWikipediaSource = async (prefName) => {
    ...
    for (const line of lines) {
        ...
        const files = line.matchAll(/\[\[(?:File|Image|ファイル):([^\]|]+)(?:\|[^\]]+)?\]\]/gi);
...
const extractPlaceImage = (content) => {
    {
...
const getKotobankCityDescription = async (city) => {
...
const getChakuwikiCityDescription = async (city) => {
    ...
    const chakuwikiApiUrl = `https://chakuwiki.org/w/api.php?${querystring_1.default.encode({
        format: 'json',
        action: 'query',
        prop: 'revisions',
        rvprop: 'content',
...
const getRandomCitySymbol = async (prefecturesFilter) => {
    const prefecturesList = prefecturesFilter ?? Object.keys(prefectures_1.prefectures);
    if (prefecturesFilter === null) {
    ...
    const prefectureChosen = (0, lodash_1.sample)(prefecturesList);
    const citySymbols = await getWikipediaSource(prefectureChosen);
    const citySymbol = (0, lodash_1.sample)(citySymbols);
    const cityInformation = await getCityInformation(citySymbol);
    return { ...citySymbol, ...cityInformation };
};
const getWikimediaImageUrl = (fileName) => {
    if (fileName.startsWith('http')) {
        return fileName;
    }
    return `https://commons.wikimedia.org/wiki/Special:FilePath/${querystring_1.default.escape(fileName)}?width=200`;
};
const getCorrectAnswers = (city) => ([
...
const generateAiHints = async (city) => {
    ...
    try {
        ...
        const concealedHints = hints.map((hint) => {
            let hintString = hint;
...
class CitySymbolAteQuiz extends atequiz_1.AteQuiz {
    #hasPrefHint;
    constructor(options) {
        super(options.slack, options.problem, options.postOptions);
        this.#hasPrefHint = options.citySymbolsOptions.hasPrefHint;
    }
        ...
        mutex.runExclusive(async () => {
            try {
                ...
                if (message.text &&
                    ...
                    if (prefectureSpecified && !Object.hasOwn(prefectures_1.prefectures, prefectureSpecified)) {
                    ...
                    const needsPrefHint = !prefectureSpecified;
                    const city = await getRandomCitySymbol(prefectureSpecified ? [prefectureSpecified] : null);
                    const quizText = 'この市区町村章ど～こだ？';
                    const imageUrl = getWikimediaImageUrl((0, lodash_1.sample)(city.files));
                    const correctAnswers = getCorrectAnswers(city);
                    const aiHintsLoader = new utils_1.Loader(() => generateAiHints(city));
                    aiHintsLoader.load();
                    const aiHintSources = [];
                    ...
                    const problem = {
                        ...
                        solvedMessage: {
                            channel: message.channel,
                            text: atequiz_1.typicalMessageTextsGenerator.solved(` ＊${city.prefectureName}${city.cityName}＊ `),
                        },
                        unsolvedMessage: {
                            channel: message.channel,
                            text: atequiz_1.typicalMessageTextsGenerator.unsolved(` ＊${city.prefectureName}${city.cityName}＊ `),
                        },
                        get answerMessage() {
                            const aiHints = aiHintsLoader.get() ?? [];
                    ...
                    if (result.state === 'solved') {
                        await (0, achievements_1.increment)(result.correctAnswerer, 'city-symbol-answer');
                        if (hintCount === 0) {
                            await (0, achievements_1.increment)(result.correctAnswerer, 'city-symbol-answer-no-hint');
                        }
                        else if (hintCount <= 1) {
                            await (0, achievements_1.increment)(result.correctAnswerer, 'city-symbol-answer-no-chatgpt-hint');
                        }
                        if (city.cityName === '博多市') {
                            await (0, achievements_1.increment)(result.correctAnswerer, 'city-symbol-answer-hakatashi');
                        }
                        else {
                            await (0, achievements_1.increment)(result.correctAnswerer, `city-symbol-answer-${city.prefectureName}`);
                            if (hintCount === 0) {
                                await (0, achievements_1.increment)(result.correctAnswerer, `city-symbol-answer-no-hint-${city.prefectureName}`);
                            }
                            else if (hintCount <= 1) {
                                await (0, achievements_1.increment)(result.correctAnswerer, `city-symbol-answer-no-chatgpt-hint-${city.prefectureName}`);
                            }
                        }
                    }
                }
